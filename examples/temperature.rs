...
use esp_idf_experiments::temp::{TempCalibration, TempConverter};
use esp_idf_experiments::{http::ntfy_send, misc::FontSize};
use esp_idf_svc::{
        ...
        let client = {
            ...
            match connect_wifi(&mut wifi) {
                Ok(()) => {
                    ...
                    Some(Client::wrap(EspHttpConnection::new(&config)?))
                }
                Err(e) => {
                    println!("{:#?}", e);
                    None
